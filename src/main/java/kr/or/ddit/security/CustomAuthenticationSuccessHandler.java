...
public class CustomAuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		EmployeeVO employee = checkUserLoginFlag(authentication.getName());
		session.setAttribute("empDetail", employee);
		...
		if(isAdmin) {
			...
			if("N".equals(employee.getEmpLoginFlag())) { //로그인 직후 비밀번호 변경이 필요한 직원인 경우
	...
	private EmployeeVO checkUserLoginFlag(String empCd) {
